...
CREATE TABLE IF NOT EXISTS USERS.PRIVILEGE(
    ID BIGSERIAL PRIMARY KEY,
    NAME VARCHAR(150),
    CREATED_BY  VARCHAR(50),
    CREATED_DATETIME    TIMESTAMP WITH TIME ZONE,
...
CREATE TABLE IF NOT EXISTS USERS.ROLES_PRIVILEGES(
   ROLE_ID INT,
   PRIVILEGE_ID INT,
   CONSTRAINT ROLE_ID FOREIGN KEY (ROLE_ID) REFERENCES USERS.ROLE(ID),
   CONSTRAINT PRIVILEGE_ID FOREIGN KEY (PRIVILEGE_ID) REFERENCES USERS.PRIVILEGE(ID)
)